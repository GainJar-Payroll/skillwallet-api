...
/**
 * test/proof/proof-ai-dca.ts
 *
 * Proof for the AI-Powered DCA skill (ai-powered-dca-84532).
 *
 * Same flow as proof.ts but:
 *   1. Targets the AI-Powered DCA skill specifically
 *   2. Uses "every second" cron expression for instant admin trigger
 *   3. Validates that AI context (aiContext, newsContext) from pre-execution
 *      Venice AI analysis is stored in the execution record
 *
 * Pre-execution AI analysis (in cron.runner.service.ts):
 *   - Fetches crypto news via x402 OttoAI (non-critical, errors don't block)
 *   - Analyzes market sentiment via Venice AI
 *   - Passes aiContext + newsContext to executeInstallation()
 *   - Stored in execution record via initRecord()
 *
 * Required env:
 *   PORT
 *   ADMIN_API_KEY
 ...
 * - SKILL_ID_TARGET defaults to ai-powered-dca-84532, override via env.
 */
...
const SKILL_ID_TARGET = process.env.SKILL_ID_TARGET ?? 'ai-powered-dca-84532';
...
function selectAiDcaSkill(skills: any[]) {
  // Prefer the AI-Powered DCA skill
  const aiSkill = skills.find(
    (skill) => skill.skillId === SKILL_ID_TARGET,
  );
  if (aiSkill) return aiSkill;

  // Fallback to Custom Cron DCA if AI skill not seeded yet
  const customCronDca = skills.find(
    (skill) => skill.skillId === `custom-cron-dca-${DEFAULT_CHAIN_ID}`,
  );
  if (customCronDca) return customCronDca;
...
async function main() {
  step('Proof: AI-Powered DCA — prepare → confirm → admin trigger → verify AI context');
  ...
  step('4. GET /skills and select AI-Powered DCA skill');
  ...
  const selectedSkill = selectAiDcaSkill(skills);
  ...
  // Warn if we fell back to a non-AI skill
  if (skillId !== SKILL_ID_TARGET) {
    log('SKILL_FALLBACK_WARNING', {
      expected: SKILL_ID_TARGET,
      actual: skillId,
      note: 'AI-Powered DCA not found — using fallback skill. AI context will not be tested.',
    });
  }
  ...
  const hasCronParam = skillDefinitions.some((def) => def.key === 'cronSchedule');

  const dcaParameters = hasCronParam
    ? [
        { key: 'cronSchedule', value: '*/1 * * * * *' },
  ...
  step('13. Verify AI context in execution record');

  const latestExecution = latestExecutionOf(finalInstallation);

  log('AI_CONTEXT_EXECUTION_CHECK', {
    installationId,
    hasAiContext: latestExecution?.aiContext !== undefined && latestExecution?.aiContext !== null,
    hasNewsContext:
      latestExecution?.newsContext !== undefined && latestExecution?.newsContext !== null,
    aiContextLength: latestExecution?.aiContext?.length ?? 0,
    newsContextLength: latestExecution?.newsContext?.length ?? 0,
    executionStatus: latestExecution?.status,
  });

  if (latestExecution?.aiContext) {
    const preview =
      latestExecution.aiContext.length > 200
        ? `${latestExecution.aiContext.slice(0, 200)}...`
        : latestExecution.aiContext;
    log('AI_CONTEXT_PREVIEW', { preview });
  }

  if (latestExecution?.newsContext) {
    const preview =
      latestExecution.newsContext.length > 200
        ? `${latestExecution.newsContext.slice(0, 200)}...`
        : latestExecution.newsContext;
    log('NEWS_CONTEXT_PREVIEW', { preview });
  }

  // AI context may be absent if x402 news fetch failed (non-fatal).
  // This is expected behaviour — we log it, not fail.
  const skillIsAi = skillId === SKILL_ID_TARGET;
  if (skillIsAi) {
    if (!latestExecution?.aiContext && !latestExecution?.newsContext) {
      log('AI_CONTEXT_MISSING_NOTE', {
        note: 'AI context is empty. This is expected if x402 OttoAI news fetch failed (non-fatal per cron.runner.service.ts).',
        reason: 'x402 requires a sponsor with USDC balance. The pre-execution AI analysis is best-effort.',
      });
    } else {
      log('AI_CONTEXT_PRESENT', {
        message: 'Pre-execution AI analysis successfully stored in execution record.',
        fields: Object.keys(latestExecution).filter(
          (k) => k === 'aiContext' || k === 'newsContext',
        ),
      });
    }
  } else {
    log('AI_CONTEXT_SKIPPED', {
      note: `Not targeting ${SKILL_ID_TARGET} — fell back to ${skillId}. AI context was not expected.`,
    });
  }
  ...
  log('SUMMARY', {
    ...
    skillIsAi,
    aiContextPresent:
      latestExecution?.aiContext !== undefined && latestExecution?.aiContext !== null,
    newsContextPresent:
      latestExecution?.newsContext !== undefined && latestExecution?.newsContext !== null,
    latestExecution,
    balances: {
