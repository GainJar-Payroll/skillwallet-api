...
export class WalletPermissionRequestRecord {
  ...
  @Prop({ required: true, enum: ['wallet_requestExecutionPermissions', 'wallet_grantPermissions', 'metamask_requestExecutionPermissions'] }) method!: string;
  @Prop({ required: true, enum: ['prepared', 'submitted', 'approved', 'rejected', 'failed'], index: true }) status!: string;
...
export const WalletPermissionRequestSchema = SchemaFactory.createForClass(WalletPermissionRequestRecord);
